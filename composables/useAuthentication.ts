...
export function useAuthentication() {
  ...
  const userToken = useHashedCookie<string | null | undefined>(
    "c96434292aae604"
  );
  ...
  const setUserCookies = (user: any, token: string) => {
  ...
  const login = async () => {
    try {
      ...
      setUserCookies(response.user, response.token); // Store user in cookies
  ...
  const register = async () => {
    try {
      ...
      const { success, message } = await $fetch("/api/auth/register", {
        method: "POST",
        body: {
  ...
  const resetPassword = async () => {
    try {
      const response = await $fetch("/api/auth/reset-password", {
        method: "POST",
        body: resetPasswordFormState.value,
      });
